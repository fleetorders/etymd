...
| `etymd propose`                  | Score the sweep's improvement findings + recurring classes against a fleet-authored rubric — stable `proposal/1` records, read-only, deterministic, corp entries excluded. `--rubric <file>` (required), `--manifest <file>` or `--from <fleet.json>`, `--json`. |
...
status | next | effort | projects | depends-on |`, the one hand-edited fleet-level surface), and
  totals; corp entries never appear on it. Day-precision stamp, deterministic output, exit code 1
...
class, recomputed over personal projects only — **corp entries are excluded from the output
