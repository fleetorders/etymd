...
```jsonc
{
  "machineProfile": "guarded", // "personal" resolves guarded entries disclosed-absent
...
- **Wall checks.** Guarded contract files found inside a guarded worktree, unregistered checkouts
  ...
  guarded-host commit emails on personal entries — each a risk finding; each check that cannot run
  is disclosed.
