...
export type FleetProfile = "personal" | "guarded"
...
const REBUILD_RECIPE =
  'rebuild it beside the manifest: {"machineProfile":"guarded","dirs":{"<name>":"~/path/to/worktree"}}'
...
function loadRegistryEntries(
  ...
  for (const [i, rawEntry] of projects.entries()) {
    ...
    const profile: FleetProfile = rec.profile === "guarded" ? "guarded" : "personal"
    if (rec.profile !== undefined && rec.profile !== "guarded" && rec.profile !== "personal") {
      manifest.problems.push({
        kind: "bad-shape",
        file: manifestFile,
    ...
    if (entry.profile === "guarded" || entry.private) {
      // Real guarded directories live ONLY in the gitignored local file — a tracked path on a
      ...
      if (!manifest.localPresent) {
        ...
        entry.unresolved =
          'guarded entries do not resolve under machineProfile "personal" — absent on this machine by design'
      } else {
...
export async function loadFleetManifest(manifestPath: string): Promise<FleetManifest> {
  ...
  if (Array.isArray(parsed.projects)) {
    ...
    manifest.guardedHosts = Array.isArray(local.data.guardedHosts)
      ? local.data.guardedHosts.filter((h): h is string => typeof h === "string")
      : []
    manifest.machineProfile =
      local.data.machineProfile === "guarded" || local.data.machineProfile === "personal"
