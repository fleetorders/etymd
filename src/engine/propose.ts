...
export function buildProposals(
  ...
  const guarded = sweep.projects.filter((p) => p.profile !== "personal")
  if (guarded.length) {
    disclosures.push(
      `guarded entries excluded from proposals: ${guarded
        .map((p) => p.name)
        .sort()
        .join(", ")}`,
    )
  ...
  // list counts guarded projects, and a class held open by guarded repos alone is not this fleet's
