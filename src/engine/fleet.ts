...
/** The wall artifacts a guarded WORKTREE must not carry at its root (they live hive-side). */
...
export function guardedPersistenceRoot(manifest: FleetManifest, name: string): string {
  return path.join(manifest.dir, "guarded", name)
}
...
/** True when an email's domain is (a registrable suffix of) one of the guarded hosts. */
export function emailMatchesGuardedHosts(email: string, hosts: string[]): boolean {
  const domain = email.split("@")[1]?.toLowerCase()
  if (!domain) return false
  return hosts.some((h) => {
    const host = h.toLowerCase()
    if (domain === host) return true
    // The email's domain may sit above the host (git.guarded.example → guarded.example) or below it.
...
export async function checkManifest(
  ...
  for (const entry of manifest.entries) {
    ...
    if (
      ...
      (entry.profile === "guarded" || entry.private)
    ) {
      findings.push(
        finding(
      ...
      if (await pathExists(path.join(entry.resolvedRoot, artifact))) {
        findings.push(
          finding(
            ...
            "Agent artifacts for guarded repos live outside the repo by policy — an in-repo copy is one push away from colleagues' eyes and will diverge from the hive-side truth.",
...
function expandNeedleDir(dir: string): string {
  // Basenames only — "~/projects/some-guarded-dir" needles as "some-guarded-dir".
  return dir.replace(/\/+$/, "")
}

async function checkGuardedEmails(
  manifest: FleetManifest,
  findings: Finding[],
  disclosures: string[],
): Promise<void> {
...
async function sweepEntry(
  ...
  // Persistence rules, pinned by test: a guarded WORKTREE is never written under any flag — its
  // ledger lives at the manifest's guarded/<name>/. A personal repo persists only when it already
  ...
  const result = await runAudit(root, {
    ...
    ledgerRoot: guarded ? guardedPersistenceRoot(manifest, entry.name) : undefined,
...
export async function fleetLedgerTarget(
  manifest: FleetManifest,
  name: string,
): Promise<{ entry: FleetEntry; auditRoot: string; ledgerRoot: string; guarded: boolean }> {
 ...
 * when it is not (guarded: audit read-only, ledger persisted hive-side). Returns false when the
...
export async function ensureFindingRecorded(
  ...
  await runAudit(target.auditRoot, {
    ...
    ledgerRoot: target.guarded ? target.ledgerRoot : undefined,
