...
export async function sweep(opts: FleetSweepCmdOptions): Promise<void> {
  ...
  const result = await sweepFleet(manifest, {
    only: opts.only,
    profile: opts.profile as "personal" | "guarded" | undefined,
 ...
 * what keeps guarded-side names out of a file that gets pushed — and it also means the entry cannot
 ...
 *
 * The gitignore check is not defensive padding. This file is the one place real guarded-side
 ...
 */
async function recordGuardedMapping(
  ...
  if (insideRepo) {
    ...
    if (!ignored) {
      throw new Error(
        `${path.basename(manifest.localPath)} is not gitignored — refusing to write guarded-side ` +
...
export async function add(opts: FleetAddCmdOptions): Promise<void> {
  ...
  // Guarded entries take their answer from the profile (machine-pinned, never publishable), so the
  ...
  // The local mapping goes FIRST for a guarded entry, because the two failure modes are not
