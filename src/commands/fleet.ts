...
export async function board(opts: FleetBoardCmdOptions): Promise<void> {
  ...
  for (const entry of manifest.entries) {
    // Corp entries never reach a board: the board is a publishable artifact of the personal
    // side, and a corp plan is exactly the content the wall exists to keep out of it.
    if (entry.profile === "corp") continue
