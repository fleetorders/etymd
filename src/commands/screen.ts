 ...
 * words that indicate a leaky DESCRIPTION rather than a secret string — ordinary English a
 * natural-language file can contain innocently. They are skipped only on files where that
 * innocence is grounded: a file proven upstream-owned (byte-identical to public upstream, see
 * `--upstream`) or a path declared GENERATED DATA in the allow file (regenerated from sources
 * of its own, so hand-editing a leaked string out of it is not even possible). Secret-class
 * patterns are absolute — they never skip, on any file; the machine-path check skips only on
 * upstream-owned files, whose bytes are public wherever they name.
 */
 ...
 *   author owner
 *
 * A `generated` line opens the other record kind — a PATH declaration, not a line exemption:
 *
 ...
 *   author owner
 *
 * It marks matching paths as GENERATED DATA: the file is still read and still screened, but
 * vocabulary-class patterns are dropped for it, because a regenerated natural-language corpus
 * trips English credential words on a schedule and every such refusal trains the habit of
 * passing `--no-verify` on a security hook. Secret-class patterns and the machine-path check
 * stay — a generated file is exactly where a real credential would be least visible, so the
 * exemption is exactly as wide as the words a corpus cannot help containing. The paths that
 * took the exemption are NAMED in the output, so a clean run never reads as fully screened.
 * A path record always needs provenance (it is never a repo naming itself), and it is matched
 * against the repo-relative path, in the staged, tree and dir scopes.
 *
...
/** The `generated <path>` record: a PATH declaration, not a line exemption. */
...
/** Everything the allow file declares: line exemptions and generated-data paths. */
...
const LABELED_LINE = /^(pattern|generated|reason|date|author)[ \t]+(.*)$/

/**
 * Compile one pattern, matching malformed input literally rather than dropping it. Shared by
 * both record kinds — a malformed value must never be a silent no-op in a file whose whole
 * job is to open holes in a gate that are at least VISIBLE ones.
 */
function compileLooseRegex(source: string): RegExp {
  try {
 ...
 *
 * A `pattern` line (or any unlabeled line — that is the self-name shorthand) opens a line
 * exemption; a `generated` line opens a path declaration; `reason`/`date`/`author` lines fill
 * whichever record is open. Records missing their provenance are RETURNED, not dropped — the
 * run-level validation reports them, so an incomplete entry is noise to fix rather than
 * silence to trust.
 *
...
export function compileAllow(raw: string): AllowFile {
  ...
  let currentGen: GeneratedEntry | null = null
  const close = () => {
...
export function screenText(
  ...
  // upstream, so any path it names is already public. Secret-class patterns still never skip. A
  // generated-data path drops the vocabulary class for the same word-innocence reason, but KEEPS
  // the machine-path check: its bytes are not proven public anywhere, and a generator that embeds
  // an absolute home path in its output's provenance metadata is exactly the quiet leak this
  // screen exists for.
  const active =
    upstreamOwned || generatedData ? classed.filter((p) => p.cls === "secret") : classed
...
export async function run(opts: ScreenOptions): Promise<void> {
  ...
  // apply, exactly like an unsigned line exemption.
  ...
  const isGenerated = (rel: string): boolean => generatedPaths.some((e) => e.path.test(rel))
  ...
  let skippedBinary = 0
  // Named, not merely counted: a file that took the generated-data exemption was screened with
  // fewer patterns, and a clean run must never read as fully screened where it was not.
  const generatedExempted: string[] = []
  ...
  section(
    `Content screen ${theme.dim(
      ...
      }${generatedExempted.length ? ` · ${generatedExempted.length} generated exempt` : ""}`,
    )}`,
  )
