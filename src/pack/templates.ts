...
if command -v shellcheck >/dev/null 2>&1; then
  # Names reach the loop as ARGUMENTS, never interpolated into the script: a tracked filename
  # carrying " or $(…) is data, not code — the -I{} shape made every push execute it.
  # shellcheck disable=SC2016  # $f belongs to the inner shell's loop, not to this one
  scripts=$(git ls-files -z \\
    | xargs -0 sh -c 'for f do head -1 "$f" 2>/dev/null | grep -qE "^#!.*[/ ](ba|da)?sh( |$)" && printf "%s\\n" "$f"; done' sh \\
    | sort)
  # shellcheck disable=SC2016  # same shape, for the zsh set
  zsh_scripts=$(git ls-files -z \\
    | xargs -0 sh -c 'for f do head -1 "$f" 2>/dev/null | grep -qE "^#!.*[/ ]zsh( |$)" && printf "%s\\n" "$f"; done' sh \\
    | sort)
