  ...
  (
    ...
    for sha in $shas; do
      ...
      if [ -e "$shellcheck_tmp/external-sources" ]; then
        ...
        while :; do
          ( cd "$tree" && find . -type f ! -name .shellcheckrc -exec sh -c 'grep -h -o -E "(^|[;&|[:space:]])(\\.|source)[[:space:]]+[^[:space:];&|]+|source=[^[:space:]]+" "$@"; st=$?; [ "$st" -le 1 ]' sh {} + ) > "$shellcheck_tmp/source-lines" \\
            && sed -E -e 's/^.*source=//' -e 's/^.*[[:space:]]//' -e "s/[\\"']//g" -e 's|.*/||' "$shellcheck_tmp/source-lines" \\
              | grep -v -e '[$]' -e '^$' | sort -u > "$shellcheck_tmp/source-names.next" || {
