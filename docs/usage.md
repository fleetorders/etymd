...
`--file <path>` for a plan, an issue, a prompt; `--file -` reads stdin) checks what the task
**names** against the repo with the same rules instruction files get, then writes a **brief**
handing the agent the premises only it can verify:
...
What counts as a claim in prose — precision over recall, as everywhere else: a path with a
directory and a recognized extension (`src/legacy/foo.test.ts`); a directory written with a
trailing slash whose first segment exists here (`src/legacy/`); a script in the `run` form
(`npm run lint`, `pnpm run build` …) or the `npm test` / `npm start` shorthands; well-known docs
(`AGENTS.md`); `D-NNN` decision ids. Anything you backtick is read exactly as an instruction file
would be. A bare `pnpm X` in a sentence, a scheme-less URL (`github.com/…`), a slash-joined phrase
...
printed and nothing is written. The file is left untracked, like `brief.md` — commit it or ignore
it as you prefer. No ledger — task findings are not remembered between runs.

Flags: `--file <path>` (`-` = stdin) · `--json` (schema `premise/1`) · `--no-brief` ·
`--fail-on <tier>`.
...
Flags: `--only <names…>` · `--profile <personal|guarded>` · `--truth` · `--persist-ledgers` ·
...
- **`etymd fleet dismiss <name> <id> --reason "…"` / `etymd fleet accept <name> <id>`** — the
  ...
  that repo's `.etymd`; guarded entries into `<manifest-dir>/guarded/<name>/.etymd/` (created there if
  missing — the guarded worktree is never touched). If the id is not yet recorded, a persisting
  single-repo audit runs internally first.
- **Persistence rules** (each pinned by test): the sweep never creates `.etymd` anywhere;
  `--persist-ledgers` applies only to personal entries that already opted in; guarded worktrees
  take zero writes under every flag combination; and after a sweep, zero guarded-resolved content
