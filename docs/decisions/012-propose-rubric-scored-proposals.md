...
1. **`etymd propose --manifest <registry.json> --rubric <file>`** (or `--from <fleet.json>` to
   ...
   writes `.etymd` anywhere, never moves `last.fleet.json`, and a guarded worktree is never
...
5. **Proposal subjects, all guarded-free:** every finding with `kind: "improvement"` from every
   personal-profile project, plus every recurring class recomputed over personal projects only
   (the ≥2-projects rule unchanged). Guarded entries are excluded from the output entire — a
   guarded improvement is the guarded side's backlog, not this fleet's proposal queue — and their
...
- Including guarded findings with a flag: a guarded worktree's backlog leaving the machine under any
  flag is the wrong default; guarded proposals do not exist in this command's world.
