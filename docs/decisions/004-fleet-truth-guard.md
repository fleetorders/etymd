...
- `registry.json` (tracked, publishable by construction): names, kinds, profiles
  (`personal | guarded`), root-relative paths for personal entries, per-entry `staleAfterDays` /
...
skip. `machineProfile: "personal"` resolves guarded entries disclosed-absent. The legacy corpus
...
- A **guarded worktree takes zero writes, ever** — regardless of flags, and even when a stray
  `.etymd` already exists inside it. Pinned by test.
- Guarded findings are still ledger-dismissible (flag fatigue is what kills reports): their
  persistence root is `<manifestDir>/guarded/<name>/.etymd/`, beside the manifest, inside the zone
  that never pushes.
- Partition invariant, pinned by test: after a sweep over guarded entries, zero guarded-resolved
...
2. Manifest loader + `fleet` / `fleet check` / `fleet dismiss|accept` + wall findings — 0.2.0,
   this document.
3. Sidecar `contractDir` audits: a manifest-side contract's claims verified against the guarded repo
   root it describes.
