...
`guarded || private`, three test `guarded` alone. Two latent properties, named:
...
**`profile` leaves the entry schema entirely** — no `personal`, no `guarded`, no replacement word.
