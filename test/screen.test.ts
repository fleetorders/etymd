...
describe("allow-file records", () => {
  it("reads a labeled record: the pattern is the rest of its line, verbatim", () => {
    const entries = compileAllow(
      ["pattern ^AcmeCorp$", "reason fixture text", "date 2026-08-15", "author nightly"].join("\n"),
    )
    expect(entries).toHaveLength(1)
    expect(entries[0]?.pattern.source).toBe("^AcmeCorp$")
  ...
  it("PINNED: a pattern may contain pipes — alternation survives intact", () => {
    ...
    const entries = compileAllow(
      ["pattern AcmeCorp|BetaCorp", "reason either name", "date 2026-08-15", "author nightly"].join(
        "\n",
      ),
    )
    expect(entries).toHaveLength(1)
    expect(entries[0]?.pattern.source).toBe("AcmeCorp|BetaCorp")
    // Both branches of the alternation suppress, and nothing wider does.
    expect(screenText("BetaCorp ships it", "f", [/GammaCorp/i], entries)).toEqual([])
    expect(screenText("AcmeCorp ships it", "f", [/GammaCorp/i], entries)).toEqual([])
    expect(screenText("GammaCorp in plain prose", "f", [/GammaCorp/i], entries)).toHaveLength(1)
  })
  ...
  it("a pattern with spaces runs to the end of the line, not to the first space", () => {
    const entries = compileAllow(
      [
        "pattern AcmeCorp and its friends",
        "reason prose pattern",
        "date 2026-08-15",
        "author x",
      ].join("\n"),
    )
    expect(entries[0]?.pattern.source).toBe("AcmeCorp and its friends")
    expect(screenText("AcmeCorp and its friends together", "f", [/AcmeCorp/i], entries)).toEqual([])
  })
})
...
describe("binary files are skipped, not screened", () => {
  ...
  it("reports a file carrying a NUL byte as binary rather than screening its bytes", async () => {
    ...
    await fs.writeFile(
      f,
      Buffer.concat([Buffer.from("\x89PNG\r\n"), Buffer.from([0]), Buffer.from("AcmeCorp")]),
    )
  ...
  it("still returns text for a file with no NUL, so the gate does not go blind", async () => {
    const f = tmp()
    await fs.writeFile(f, "Copyright (c) AcmeCorp\n")
