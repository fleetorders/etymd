...
describe("propose — the fixture fleet (acceptance)", () => {
  ...
  it("excludes corp entries from the output entire — findings, classes, and by name", async () => {
    await initHeavyRepo("alpha")
    await initHeavyRepo("beta")
    await write("rubric.txt", RUBRIC)
    const manifest = await writeHub([
      personal("alpha"),
      personal("beta"),
      { name: "c-one", kind: "repo", profile: "corp", private: true, contract: {}, links: {} },
    ])

    const result = await proposeJson(manifest)
    expect(result.proposals.some((p) => p.projects.includes("c-one"))).toBe(false)
    expect(result.disclosures.some((d) => /corp entries excluded.*c-one/.test(d))).toBe(true)
  })
})
...
describe("propose — subject selection (synthetic sweeps)", () => {
  ...
  it("a class held open only by corp projects is not this fleet's proposal", () => {
    const result = buildProposals(
      sweep([
        project("c-one", "corp", [{ id: "context-economy/heavy-file:AGENTS.md" }]),
        project("c-two", "corp", [{ id: "context-economy/heavy-file:AGENTS.md" }]),
      ]),
      "r.txt",
      rubric,
    )
